...
from PyQt6.QtWidgets import (
    QApplication, QGridLayout, QMainWindow, QLabel, QPushButton,
    QVBoxLayout, QWidget, QListWidget, QFileDialog, QMessageBox,
    QDialog, QLineEdit, QHBoxLayout, QProgressBar
)
...
def log_action(action, posture_status=None, back_angle=None, water_intake=None, break_taken=None, activity=None, forward_lean=None, shoulder_alignment=None, session_status=None, game=None):
    try:
        with sqlite3.connect("health_tracker.db") as conn:
            ...
            conn.commit()
            print(f"Logged action: {action}")  # Debug statement
    except sqlite3.Error as e:
        print(f"Database error: {e}")
    # main_window.update_logs()
...
class Overlay(QMainWindow):
    def __init__(self):
        ...
        self.video_label.setStyleSheet("QLabel { border-radius: 20px; background-color: #C8E0EC; padding: 10px; border:5px solid #C8E0EC;}")
        ...
        self.posture_feedback.setStyleSheet("QLabel { background-color: grey; border-radius: 20px; padding: 10px; font-size: 20px; color: white; font-weight: bold; }")

        self.game_status = QLabel("Game Status: Not Running")
        self.left_layout.addWidget(self.game_status)
        self.game_status.setStyleSheet("QLabel { font-size: 20px; color: grey; font-weight: bold; }")
        ...
        self.label.setStyleSheet("QLabel { padding: 5px; font-size: 20px; color: Black; font-weight: bold; }")
        ...
        self.start_button.setStyleSheet("""
            QPushButton { background-color: #4CAF50; color: white; border-radius: 15px; padding: 10px; font-size: 14px; font-weight: bold; border: 2px solid #4CAF50; }
        ...
        self.stop_button.setStyleSheet("""
            QPushButton { background-color: #f44336; color: white; border-radius: 15px; padding: 10px; font-size: 14px; font-weight: bold; border: 2px solid #f44336; }
        ...
        self.logs_label.setStyleSheet("QLabel { background-color: grey; border-radius: 10px; padding: 5px; font-size: 18px; color: black; font-weight: bold; }")
        ...
        self.settings_button.setStyleSheet("""
            QPushButton { background-color: #2196F3; color: white; border-radius: 15px; padding: 10px; font-size: 14px; font-weight: bold; border: 2px solid #2196F3; }
        ...
        self.progress_button.setStyleSheet("""
            QPushButton { background-color: #9C27B0; color: white; border-radius: 15px; padding: 10px; font-size: 14px; font-weight: bold; border: 2px solid #9C27B0; }
        ...
        self.export_button.setStyleSheet("""
            QPushButton { background-color: #FF9800; color: white; border-radius: 15px; padding: 10px; font-size: 14px; font-weight: bold; border: 2px solid #FF9800; }
        ...
        self.dark_mode_button.setStyleSheet("""
            QPushButton { background-color: #607D8B; color: white; border-radius: 15px; padding: 10px; font-size: 14px; font-weight: bold; border: 2px solid #607D8B; }
    ...
    def start(self):
        if not self.running:
            ...
            # Initialize the posture detector and video stream again when starting
            if not self.posture_detector.initialize_camera():
                QMessageBox.critical(self, "Error", "Could not initialize camera")
                return
            self.video_label.setText(" Hello Gamer")
            self.video_label.setStyleSheet("QLabel { border-radius: 20px; background-color: white; padding: 10px;font-weight:Bold; font-size:30px; }")
    ...
    def stop(self):
        if self.running:
            self.running = False
            self.posture_detector.release()  # Properly release the camera
            log_action("Session stopped", session_status="Stopped")
            # Ensure the video update loop stops
            self.video_label.clear()
            self.video_label.setText("Adios")
            self.video_label.setStyleSheet("QLabel { border-radius: 20px; background-color: white; padding: 10px;font-weight:Bold; font-size:30px; }")
    ...
    def update_timer(self):
        while self.running:
            ...
            self.game_status.setText(game_status_text)
            self.update_logs()
            time.sleep(0.1)
    ...
    def update_logs(self):
        try:
            with sqlite3.connect("health_tracker.db") as conn:
                ...
                rows = c.fetchall()
                print("Fetched logs from database:", rows)  
        except sqlite3.Error as e:
            print(f"Database error: {e}")
            rows = []
        ...
        print("Updated log list in UI")  # Debug statement
